...
public class Connection : IDisposable
{
    ...
    public bool TryMakeConnection(Server server, NetDataWriter connectionData, bool reconnect = false)
    {
        ...
        ProxyLogger.Info($"{Client.Tag} {(reconnect ? "Reconnecting" : "Connecting")} to (f=yellow){server.Name}(f=white)", "Client");
    ...
    void OnDisconnected(NetPeer peer, DisconnectInfo disconnectInfo)
    {
        ...
        {
            default:
                ProxyLogger.Info($"{Client.Tag} {disconnectInfo.Reason}", "Client");
                break;
            case DisconnectReason.ConnectionFailed when disconnectInfo.AdditionalData.RawData == null:
                ...
                ProxyLogger.Info($"{Client.Tag} Server (f=yellow){Server.IpAddress}:{Server.Port}(f=white) is offline!", "Client");
            ...
            case DisconnectReason.ConnectionRejected when disconnectInfo.AdditionalData.RawData != null:
                ...
                switch (reason)
                {
                    case RejectionReason.Delay:
                        ...
                        ProxyLogger.Info($"{Client.Tag} Delay connecting to (f=yellow){Server.IpAddress}:{Server.Port}(f=white) by {offset} seconds!", "Client");
                        break;
                    ...
                    case RejectionReason.ServerFull:
                        ...
                        ProxyLogger.Info($"{Client.Tag} Server (f=yellow){Server.IpAddress}:{Server.Port}(f=white) is full!", "Client");
                    ...
                    case RejectionReason.Banned:
                        ...
                        ProxyLogger.Info($"{Client.Tag} Banned from (f=yellow){Server.IpAddress}:{Server.Port}(f=white) with reason (f=yellow){banReason}(f=white)!", "Client");
                        break;

                    case RejectionReason.Challenge:
                        ProxyLogger.Info($"{Client.Tag} Processing challenge.", "Client");
            ...
            case DisconnectReason.PeerNotFound:
                ProxyLogger.Info($"{Client.Tag} Timeout!", "Client");
                return;
            ...
            case DisconnectReason.RemoteConnectionClose:
                ...
                ProxyLogger.Info($"{Client.Tag} Remote server closed connection!", "Client");
    ...
    void AcceptConnection()
    {
        ...
        if (Client.Object != null)
        {
            ...
            ProxyLogger.Info("Send FastRoundRestart packet");
            Client.FastRoundrestart();
            ProxyLogger.Info("Send NotReady packet");
            Client.NotReady();
        }
    }

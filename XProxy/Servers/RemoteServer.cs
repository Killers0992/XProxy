...
public class RemoteServer : Server
{
    ...
    public ServerSpecificSettingBase[] ServerSettings
    {
        get
        {
            if (_settings == null)
            {
                ...
                int id = 0;
                foreach (string server in Settings.Singleton.ServersInSelector)
                {
    ...
    public override void OnClientSpawned(BaseClient client) => client.SendServerSpecificEntries(ServerSettings);
    public override void OnClientSSSReponse(BaseClient client, int id)
    {
